...
namespace HotelBookingAPI.Controllers
{
    ...
    {
        private readonly RoomRepository _roomRepositry;
        ...
        {
            _logger.LogInformation("Request recieved for GetAllRooms: {@GetAllRoomsRequestDTO}", request);
            ...
            try
            {
                var rooms = await _roomRepositry.GetAllRoomsAsync(request);
        ...
        {
            _logger.LogInformation("Request recieved for GetRoomByID, id : {id}", id);

            try
            {
                var response = await _roomRepositry.GetRoomByIdAsync(id);
        ...
        public async Task<APIResponse<CreateRoomResponseDTO>> CreateRoom([FromBody] CreateRoomRequestDTO request)
        {
            ...
            try
            {
                var response = await _roomRepositry.CreateRoomAsync(request);
        ...
        public async Task<APIResponse<UpdateRoomResponseDTO>> UpdateRoom(int id, [FromBody] UpdateRoomRequestDTO request)
        {
            ...
            try
            {
                var response = await _roomRepositry.UpdateRoomAsync(request);
        ...
        public async Task<APIResponse<DeleteRoomResponseDTO>> DeleteRoom(int id)
        {
            ...
            try
            {
                var room = await _roomRepositry.GetRoomByIdAsync(id);
                if (room == null)
                {
                    return new APIResponse<DeleteRoomResponseDTO>(HttpStatusCode.NotFound, "Room not found.");
                }
                var response = await _roomRepositry.DeleteRoomAsync(id);
